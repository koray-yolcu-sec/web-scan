...
from ..models.scan_result import ScanResult, Finding
import json
...
class SecurityReporter:
    ...
    def generate_terminal_report(self, result: ScanResult):
        ...
        # Başlık
        title = f"🔒 TR-Pasif Web Güvenlik Skoru Raporu"
        if self.shark_mode:
            title += " 🦈 (Shark Mode)"
    ...
    def _print_quick_wins(self, result: ScanResult):
        ...
        table = Table(title="⚡ Hızlı Kazanımlar (Quick Wins)", show_header=True, header_style="bold cyan")
        table.add_column("#", style="dim", width=3)
    ...
    def _print_findings(self, result: ScanResult):
        ...
        for severity in ['Kırmızı', 'Sarı', 'Yeşil']:
            ...
            self.console.print(f"🔴 {severity} Bulgular ({len(findings)})", style=f"bold {severity_style}")
...
"""
    ...
    def _print_score_meaning(self, result: ScanResult):
        ...
        panel = Panel(
            ...
            title="💡 Bu Skor Ne Anlama Geliyor?",
            border_style="blue"
        )
...
"""
    ...
    def generate_markdown_report(self, result: ScanResult) -> str:
        ...
        # Başlık
        md_lines.append(f"# 🔒 TR-Pasif Web Güvenlik Skoru Raporu")
        if self.shark_mode:
        ...
        md_lines.append(f"## 💯 Güvenlik Skoru\n")
        md_lines.append(f"{score_emoji} **{result.score.score}/100** ({result.score.color} - {result.score.label})\n")
        
        # Skor anlamı
        md_lines.append(f"### 💡 Bu Skor Ne Anlama Geliyor?")
        md_lines.append(f"{result.score.meaning}\n")
        
        # Önce bunları düzelt
        if result.top_priorities:
            md_lines.append(f"## 🎯 Önce Bunları Düzelt\n")
            for i, priority in enumerate(result.top_priorities, 1):
        ...
        if result.quick_wins:
            md_lines.append(f"## ⚡ Hızlı Kazanımlar (Quick Wins)\n")
            for i, win in enumerate(result.quick_wins, 1):
        ...
        if result.findings:
            ...
            for severity in ['Kırmızı', 'Sarı', 'Yeşil']:
                ...
                for i, finding in enumerate(findings, 1):
                    ...
                    md_lines.append(f"**Önem:** {finding.severity} (-{abs(finding.score_impact)})\n")
                    md_lines.append(f"**Etki:** {finding.description}\n")
                    md_lines.append(f"**Kanıt:**")
                    md_lines.append(f"```\n{finding.evidence}\n```\n")
                    md_lines.append(f"**Çözüm:**")
            ...
            md_lines.append(f"## ✅ Güvenlik Bulguları\n")
            md_lines.append("Kritik güvenlik sorunu bulunamadı!\n")
        
        # Kategori özeti
        md_lines.append(f"## 📊 Kategori Özeti\n")
        ...
        # Ek bilgiler
        md_lines.append(f"## ℹ️  Ek Bilgiler\n")
        ...
        # Quick Fix Checklist
        md_lines.append(f"## 🛠️  Quick Fix Checklist\n")
        md_lines.append(f"### Nginx\n")
        md_lines.append(f"```nginx\n")
        md_lines.append(f"# HTTPS redirect\n")
        md_lines.append(f"server {{\n")
        md_lines.append(f"    listen 80;\n")
        md_lines.append(f"    server_name example.com;\n")
        md_lines.append(f"    return 301 https://$host$request_uri;\n")
        md_lines.append(f"}}\n\n")
        md_lines.append(f"# Security headers\n")
        md_lines.append(f"add_header Strict-Transport-Security &quot;max-age=31536000; includeSubDomains&quot; always;\n")
        md_lines.append(f"add_header X-Frame-Options &quot;SAMEORIGIN&quot; always;\n")
        md_lines.append(f"add_header X-Content-Type-Options &quot;nosniff&quot; always;\n")
        md_lines.append(f"add_header Referrer-Policy &quot;strict-origin-when-cross-origin&quot; always;\n")
        md_lines.append(f"# Content-Security-Policy kendi ihtiyacınıza göre ayarlayın\n")
        md_lines.append(f"```\n")
        
        md_lines.append(f"### Apache\n")
        md_lines.append(f"```apache\n")
        md_lines.append(f"# HTTPS redirect\n")
        md_lines.append(f"<VirtualHost *:80>\n")
        md_lines.append(f"    ServerName example.com\n")
        md_lines.append(f"    Redirect permanent / https://example.com/\n")
        md_lines.append(f"</VirtualHost>\n\n")
        md_lines.append(f"# Security headers\n")
        md_lines.append(f"<IfModule mod_headers.c>\n")
        md_lines.append(f"    Header always set Strict-Transport-Security &quot;max-age=31536000; includeSubDomains&quot;\n")
        md_lines.append(f"    Header always set X-Frame-Options &quot;SAMEORIGIN&quot;\n")
        md_lines.append(f"    Header always set X-Content-Type-Options &quot;nosniff&quot;\n")
        md_lines.append(f"    Header always set Referrer-Policy &quot;strict-origin-when-cross-origin&quot;\n")
        md_lines.append(f"</IfModule>\n")
        md_lines.append(f"```\n")
        
        md_lines.append(f"### Cloudflare\n")
        md_lines.append(f"- Cloudflare'da bu header'lar otomatik olarak eklenir\n")
        md_lines.append(f"- Transform Rules > Modify Response Header ile ekleyebilirsiniz\n")
        md_lines.append(f"- Page Rules ile HTTPS zorlama yapabilirsiniz\n")
        md_lines.append("")
        
        # Bu tool ne yapmaz
        md_lines.append(f"## ⚖️  Bu Tool Ne YAPMAZ\n")
        md_lines.append(f"- ❌ Aktif saldırı veya exploit denemeleri\n")
        md_lines.append(f"- ❌ Brute-force veya credential stuffing\n")
        md_lines.append(f"- ❌ SQLi, XSS gibi istismar testleri\n")
        md_lines.append(f"- ❌ Agresif tarama veya rate limit zorlama\n")
        md_lines.append(f"- ❌ Gizli dizin brute-force (yoğun)\n")
        md_lines.append("")
        
        md_lines.append(f"## ✅ Bu Tool Ne Yapar\n")
        md_lines.append(f"- ✅ Pasif HTTP/HTTPS analiz\n")
        md_lines.append(f"- ✅ Header ve cookie kontrol\n")
        md_lines.append(f"- ✅ DNS güvenlik kayıtları\n")
        md_lines.append(f"- ✅ HTTPS/TLS sertifika kontrol\n")
        md_lines.append(f"- ✅ Sayfa yapısı ve frontend güvenlik sinyalleri\n")
        md_lines.append(f"- ✅ Yasal ve etik sınırlar içinde kalır\n")
        md_lines.append("")
        
        # Legal uyarı
        md_lines.append(f"## ⚠️  YASAL VE ETİK UYARI\n")
        md_lines.append(f"Bu rapor sadece eğitim ve bilgilendirme amaçlıdır.\n")
        md_lines.append(f"- Bu aracı **sadece kendi sahip olduğunuz** veya **açıkça test izni aldığınız** sistemlerde kullanın.\n")
        md_lines.append(f"- Bu rapordaki bulgular profesyonel güvenlik uzmanları tarafından doğrulanmalıdır.\n")
        md_lines.append(f"- Herhangi bir izin olmadan başkasına ait sistemlerde tarama yapmak **yasa dışıdır** ve suç teşkil eder.\n")
        md_lines.append(f"- Kullanıcı bu aracı kullanarak herhangi bir yasayı veya etik kuralı ihlal etmekten tamamen sorumludur.\n")
        md_lines.append("")
