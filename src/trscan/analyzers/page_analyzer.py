...
from ..models.scan_result import Finding, PageInfo
...
class PageAnalyzer:
    ...
    def analyze(
        self, html_content: str, base_url: str, headers: dict
    ) -> tuple[PageInfo, list[Finding]]:
        """
    ...
    def _analyze_external_scripts(self, soup: BeautifulSoup, page_info: PageInfo, base_url: str):
        ...
        cdn_count = sum(
            1 for script in external_scripts if any(cdn in script for cdn in cdn_domains)
        )
    ...
    def _extract_meta_generator(self, soup: BeautifulSoup) -> str | None:
