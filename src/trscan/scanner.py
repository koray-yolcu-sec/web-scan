...
from .fetcher import HTTPFetcher, DNSFetcher
...
from .models import ScanResult, CookieInfo, Finding
...
class WebSecurityScanner:
    ...
    async def scan(self, target_url: str) -> ScanResult:
        ...
        print(f"[🍪] Cookie'ler analiz ediliyor...")
        cookie_infos, cookie_findings = self.cookie_analyzer.analyze(
            main_page_result['cookies']
        )
        all_findings.extend(cookie_findings)
        print(f"[✅] {len(cookie_infos)} cookie analiz edildi")
        
        # 3. Header'ları analiz et
        print(f"[📋] Header'lar analiz ediliyor...")
        header_analysis, header_findings = self.header_analyzer.analyze(
            main_page_result['headers']
        )
        all_findings.extend(header_findings)
        print(f"[✅] Header'lar analiz edildi")
        
        # 4. TLS/HTTPS analiz et
        print(f"[🔒] TLS/HTTPS analiz ediliyor...")
        tls_info, tls_findings = self.tls_analyzer.analyze(main_page_result)
        all_findings.extend(tls_findings)
        print(f"[✅] TLS/HTTPS analiz edildi")
        
        # 5. Sayfa yapısını analiz et
        print(f"[📄] Sayfa yapısı analiz ediliyor...")
        page_info, page_findings = self.page_analyzer.analyze(
            main_page_result['content'],
            main_page_result['final_url'],
            main_page_result['headers']
        )
        all_findings.extend(page_findings)
        print(f"[✅] Sayfa yapısı analiz edildi")
        ...
        print(f"[✅] Path'ler kontrol edildi")
        ...
        print(f"[🌐] DNS güvenlik kayıtları analiz ediliyor...")
        ...
        print(f"[✅] DNS kayıtları analiz edildi")
        
        # 8. Skor hesapla
        print(f"[💯] Güvenlik skoru hesaplanıyor...")
        
        # Score impact'ları topla
        score_impacts = [f.score_impact for f in all_findings]
        
        security_score, categories_summary, quick_wins, top_priorities = self.scorer.calculate_score(
            all_findings,
