...
Yasal Uyarı:
  ...
  sistemler üzerinde kullanın. İzinsiz tarama yasa dışıdır.
    ...
    scan_parser.add_argument(
        'url',
        help='Hedef URL (örn: https://example.com)'
    )
    ...
    scan_parser.add_argument(
        ...
        type=str,
        default=None,
        help='JSON rapor çıktı dosyası (örn: report.json)'
    )
    ...
    scan_parser.add_argument(
        ...
        type=int,
        default=15,
        help='Maksimum istek sayısı (varsayılan: 15)'
    )
    ...
    scan_parser.add_argument(
        '--no-polite',
        action='store_true',
        help='Polite mode\'u kapat (yapmamanızı öneririz)'
    )
    
    scan_parser.add_argument(
        '--shark-mode',
        action='store_true',
        help='Shark Mode 🦈 (daha katı puanlama)'
    )
    ...
    scan_parser.add_argument(
        '--no-login-paths',
        action='store_true',
        help='/login ve /admin path kontrollerini yapma'
    )
